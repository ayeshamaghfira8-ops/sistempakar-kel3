...
const KNOWLEDGE_BASE = [
  {
    id: 'GERD', name: 'GERD', emoji: '🔥', theme: 'theme-gerd',
    rules: [
      { gejala: ['GA'], bobot: 50 },
      { gejala: ['GB'], bobot: 25 },
      { gejala: ['GC'], bobot: 20 },
    ...
    saran: ['Hindari makan besar lalu langsung tidur', 'Kurangi makanan berlemak, pedas, kopi, dan alkohol', 'Tinggikan posisi kepala saat tidur', 'Konsultasi dokter untuk antasida atau PPI'],
  },
  {
    id: 'Sembelit', name: 'Sembelit', emoji: '🟤', theme: 'theme-sembelit',
    rules: [
      { gejala: ['GF'], bobot: 55 },
      { gejala: ['GG'], bobot: 40 },
      { gejala: ['GE'], bobot: 20 },
    ...
    saran: ['Minum air putih minimal 8 gelas per hari', 'Perbanyak sayur, buah, dan makanan berserat', 'Rutin olahraga ringan setiap hari', 'Jangan menahan keinginan untuk BAB'],
  },
  {
    id: 'IBS', name: 'IBS', emoji: '🌀', theme: 'theme-ibs',
    rules: [
      { gejala: ['GH'], bobot: 55 },
      { gejala: ['GI'], bobot: 35 },
      { gejala: ['GB'], bobot: 20 },
    ...
    saran: ['Identifikasi makanan pemicu dan hindari', 'Kelola stres dengan relaksasi atau olahraga', 'Konsumsi probiotik secara rutin', 'Konsultasi dokter untuk terapi yang tepat'],
  },
  {
    id: 'Apendisitis', name: 'Apendisitis', emoji: '🔴', theme: 'theme-apendisitis',
    rules: [
      { gejala: ['GJ'], bobot: 65 },
      { gejala: ['GL'], bobot: 45 },
      { gejala: ['GK'], bobot: 25 },
    ...
    saran: ['⚠️ SEGERA ke IGD rumah sakit terdekat!', 'Jangan konsumsi obat pereda nyeri sembarangan', 'Jangan menunda pemeriksaan medis', 'Penanganan utama adalah operasi pengangkatan usus buntu'],
  },
  {
    id: 'Maag', name: 'Gastritis (Maag)', emoji: '💛', theme: 'theme-maag',
    rules: [
      { gejala: ['GL'], bobot: 40 },
      { gejala: ['GB'], bobot: 40 },
      { gejala: ['GC'], bobot: 30 },
    ...
    saran: ['Makan dalam porsi kecil tapi lebih sering', 'Hindari makanan pedas, asam, kopi, alkohol, dan rokok', 'Minum antasida sesuai anjuran dokter', 'Kelola stres agar tidak memperburuk gejala'],
  },
];

const FAQ_ANSWERS = {
  gerd: '🔥 <b>GERD</b> adalah kondisi asam lambung naik ke kerongkongan secara berulang.<br><br><b>Gejala utama:</b> heartburn, asam naik, nyeri dada.<br><b>Saran:</b> Hindari makan besar lalu langsung tidur, kurangi kopi & makanan pedas, tinggikan kepala saat tidur.',
  ibs: '🌀 <b>IBS</b> (Irritable Bowel Syndrome) adalah gangguan fungsional usus besar.<br><br><b>Gejala utama:</b> nyeri perut membaik setelah BAB, diare/sembelit bergantian, kembung.<br><b>Saran:</b> Identifikasi makanan pemicu, kelola stres, konsumsi probiotik.',
  maag: '💛 <b>Maag (Gastritis)</b> adalah peradangan dinding lambung.<br><br><b>Gejala utama:</b> nyeri ulu hati, mual, perut kembung, cepat kenyang.<br><b>Saran:</b> Makan porsi kecil tapi sering, hindari makanan pedas & asam, minum antasida sesuai anjuran dokter.',
  sembelit: '🟤 <b>Sembelit</b> adalah kondisi BAB kurang dari 3x seminggu dengan feses keras.<br><br><b>Gejala utama:</b> susah BAB, perut kembung, mengejan berlebihan.<br><b>Saran:</b> Minum air putih minimal 8 gelas/hari, perbanyak serat, rutin olahraga ringan.',
  apendisitis: '🔴 <b>Apendisitis</b> adalah radang usus buntu — kondisi darurat medis!<br><br><b>Gejala utama:</b> nyeri berpindah ke perut kanan bawah, mual, demam ringan.<br>⚠️ <b>Segera ke IGD rumah sakit terdekat!</b>',
  saran: 'Berikut saran umum menjaga kesehatan pencernaan:<br><br>✅ Makan teratur & tidak terburu-buru<br>✅ Minum air putih 8 gelas/hari<br>✅ Perbanyak sayur & buah berserat<br>✅ Kelola stres dengan baik<br>✅ Hindari makanan pedas, asam, & berlemak berlebihan<br>✅ Konsultasi dokter jika gejala berlanjut',
  halo: 'Halo! 👋 Saya asisten kesehatan pencernaan.<br>Kamu bisa tanya tentang: <b>GERD, IBS, Maag, Sembelit, Apendisitis</b>, atau minta <b>saran kesehatan pencernaan</b>.',
};
...
function matchFAQ(pesan) {
  ...
  const isSaran = p.match(/\b(saran|tips|cara|mengobati|pengobatan|penanganan)\b/);
  ...
  if (isHalo && !isGerd && !isIbs && !isMaag && !isSembelit && !isApend) return FAQ_ANSWERS.halo;
  if ((isCaraMencegah || isSaran) && (isGerd || isIbs || isMaag || isSembelit || isApend)) return null;
  ...
  if (isCaraMencegah || isSaran) return FAQ_ANSWERS.saran;
  return null;
}
...
Jawab pertanyaan seputar penyakit pencernaan (GERD, IBS, Maag, Sembelit, Apendisitis) dengan bahasa Indonesia yang santai dan mudah dipahami.
Jawab singkat, padat, tanpa format bold atau penomoran. Maksimal 3-4 kalimat.
Selalu ingatkan pengguna untuk konsultasi dokter untuk diagnosis resmi.`;
...
function forwardChaining(selectedGejala) {
  ...
  KNOWLEDGE_BASE.forEach(penyakit => {
    ...
    const persentase = Math.min(Math.round((totalBobot / penyakit.maxBobot) * 100), 99);
    hasilDiagnosa.push({ id: penyakit.id, name: penyakit.name, emoji: penyakit.emoji, theme: penyakit.theme, persentase, description: penyakit.description, symptoms: penyakit.symptoms, saran: penyakit.saran });
  });
...
function renderHasil(selectedGejala, topDiagnosa) {
  ...
  if (!adaHasil) {
    pctEl.innerHTML = '<p style="color:var(--text-light); text-align:center; padding:16px;">😕 Tidak ada kecocokan gejala yang kuat. Coba pilih lebih banyak gejala.</p>';
  } else {
    topDiagnosa.forEach((d, idx) => {
      const card = document.createElement('div');
      card.className = 'pct-card ' + d.theme + (idx === 0 ? ' rank-1' : '');
      card.innerHTML = '<div class="pct-top"><span class="pct-name">' + d.emoji + ' ' + d.name + '</span><div style="display:flex; align-items:center; gap:10px;">' + (idx === 0 ? '<span class="rank-badge">🏆 Teratas</span>' : '') + '<span class="pct-value">' + d.persentase + '%</span></div></div><div class="pct-bar-bg"><div class="pct-bar-fill" style="width:0%;" data-width="' + d.persentase + '%"></div></div>';
      pctEl.appendChild(card);
    });
    requestAnimationFrame(() => {
      setTimeout(() => {
        document.querySelectorAll('.pct-bar-fill').forEach(bar => { bar.style.width = bar.dataset.width; });
      }, 120);
    });
  }
  ...
  topDiagnosa.filter(d => d.persentase > 0).forEach(d => {
    ...
    card.innerHTML = '<h4>' + d.emoji + ' What is ' + d.name + '?</h4><p>' + d.description + '</p><div class="label">Gejala Umum</div><ul>' + d.symptoms.map(s => '<li>' + s + '</li>').join('') + '</ul><div class="label">Saran & Penanganan</div><ul>' + d.saran.map(s => '<li>' + s + '</li>').join('') + '</ul>';
    explEl.appendChild(card);
  });
  if (!adaHasil) explEl.innerHTML = '<p style="color:var(--text-light); text-align:center;">Tidak ada penjelasan yang dapat ditampilkan.</p>';
}
...
async function saveGambarDiagnosa() {
  ...
  try {
    const canvas = await html2canvas(container, { scale: 2, backgroundColor: '#FFF8F3', useCORS: true, logging: false });
...
function initChatbot() {
  ...
  async function handleSend() {
    ...
    if (faqJawaban) {
      setTimeout(() => { appendMessage('bot', faqJawaban); sendBtn.disabled = false; }, 400);
      return;
    }
    showLoading();
    try {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnStart) btnStart.addEventListener('click', (e) => { e.stopPropagation(); showPage('page-gejala'); });

  const btnBackToLanding = document.getElementById('btn-back-to-landing');
  if (btnBackToLanding) btnBackToLanding.addEventListener('click', (e) => { e.stopPropagation(); showPage('page-landing'); });

  const btnDiagnosa = document.getElementById('btn-diagnosa');
  if (btnDiagnosa) {
    btnDiagnosa.addEventListener('click', () => {
      const selected = Array.from(document.querySelectorAll('input[name="gejala"]:checked')).map(cb => cb.value);
      if (selected.length === 0) {
        const msg = document.getElementById('validation-msg');
        if (msg) { msg.style.display = 'block'; msg.scrollIntoView({ behavior: 'smooth', block: 'center' }); setTimeout(() => { msg.style.display = 'none'; }, 4000); }
        return;
      }
      const msg = document.getElementById('validation-msg');
  ...
  if (btnHome) btnHome.addEventListener('click', () => { resetForm(); showPage('page-landing'); });

  const btnUlangi = document.getElementById('btn-ulangi');
  if (btnUlangi) btnUlangi.addEventListener('click', () => { resetForm(); showPage('page-gejala'); });

  const btnSave = document.getElementById('btn-save');
  if (btnSave) btnSave.addEventListener('click', saveGambarDiagnosa);
